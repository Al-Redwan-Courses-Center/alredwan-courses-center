  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] px-16 py-32 flex flex-col gap-32">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-16 border-b border-olive-100 pb-24">
        <div>
          <h1 className="text-4xl font-medad text-olive-800 tracking-tight">قائمة المعلمين</h1>
          <p className="text-olive-400 mt-4">إدارة وتوجيه الكادر التعليمي في المركز</p>
        </div>
      </div>
      ...
      <div className="grid gap-20 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 [@media(max-width:600px)]:grid-cols-1 [@media(min-width:600px)]:grid-cols-2 [@media(min-width:900px)]:grid-cols-3 [@media(min-width:1200px)]:grid-cols-4">
        {instructors.map((instructor: Instructor, index: number) => (
          <ItemCard
            ...
            className="h-full"
            cardHeader={
              <div className="relative h-40 w-full overflow-hidden bg-olive-50 grid place-items-center">
                <Avatar 
                  src={instructor.image_url} 
                  alt={instructor.name} 
                  className="h-28 w-28 border-4 border-white shadow-md z-10" 
                />
            ...
            cardFooter={
              <div className="flex justify-center pb-24 px-24">
                <Button 
                  ...
                  variant="primary" 
                  size="small" 
                  className="rounded-xl text-[1.2rem] px-32 min-w-[16rem] whitespace-nowrap"
                >
                  عرض الملف الشخصي
                </Button>
              </div>
          ...
          >
            <div className="flex flex-col gap-4 py-8">
              <div className="flex justify-between items-start">
                <h3 className="text-[1.6rem] font-bold text-olive-900 leading-tight">
                  {instructor.name}
                </h3>
                <span className={cn(
              ...
              <div className="flex items-center gap-2 text-olive-600">
                ...
                <span className="text-[1.2rem] font-medium">{instructor.type_display}</span>
              </div>
                  ...
                  {instructor.tags.slice(0, 3).map((tag, i) => (
                    <span 
                      ...
                      className={cn(
                        ...
                        i % 2 === 0 ? "rounded-[0.8rem_0]" : "rounded-[0_0.8rem]"
                      )}
                    >
                      {tag.name}
                    </span>
              ...
              <p className="mt-4 text-[1.1rem] text-gray-500 line-clamp-2 leading-relaxed">
