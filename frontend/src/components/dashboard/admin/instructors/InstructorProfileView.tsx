    ...
    <div className="bg-white/40 backdrop-blur-md border border-white/20 rounded-3xl p-24 shadow-sm flex flex-col md:flex-row gap-24">
      ...
      <div className="flex flex-col items-center gap-16 shrink-0">
        ...
        <div className="text-center">
          <h2 className="text-3xl font-medad text-olive-700">{instructor.name}</h2>
          <span className="text-olive-300 text-lg font-medium">{instructor.type_display}</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
          ...
          <InfoItem label="عدد التقييمات" value={toHindiDigits((instructor.rating_count || 0).toString())} />
        </div>

        <div className="border-t border-olive-100/50 pt-16">
          <h4 className="text-xl font-bold text-olive-600 mb-8">النبذة الشخصية</h4>
