...
class Lecture(models.Model):
    ...
    instructor = models.ForeignKey('users.Instructor', null=True, blank=True, on_delete=models.SET_NULL, related_name='lectures') # Can override course instructor
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['course', 'lecture_number'], name='unique_course_lecture'),
        ]
        indexes = [
            models.Index(fields=['course'], name='lecture_course_index'),
            models.Index(fields=['scheduled_at'], name='lecture_scheduled_at_index'),
            models.Index(fields=['course', 'lecture_number'], name='lecture_course_lecture_index'),
        ]
    ...
    def update_status(self, new_status):
        ...
        allowed_transitions = {
            Status.SCHEDULED: [Status.COMPLETED, Status.CANCELLED], # status can change from scheduled to completed or cancelled
            Status.COMPLETED: [], 
            Status.CANCELLED: [], # no transitions allowed from completed or cancelled
        }

        if new_status not in Status.values:
            raise ValidationError(f"Invalid status: {new_status}") # check if new status is valid
        if new_status not in allowed_transitions[self.status]:
            raise ValidationError(f"Cannot transition from {self.status} to {new_status}") # check if transition is allowed
