...
> **Note:** The `instructor_id` field returns the instructor's ID if the user has an instructor profile, or `null` otherwise. This is useful for the frontend to determine if the user is an instructor and fetch their instructor-specific data.
...
| Update Profile | PUT/PATCH | `/auth/users/me/` | Yes |
