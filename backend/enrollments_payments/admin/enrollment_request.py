    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "expired":
            return queryset.filter(
                expires_at__lt=now, status=EnrollmentRequestStatus.PENDING
            )
...
def approve_selected(modeladmin, request, queryset):
    ...
    approvable = queryset.filter(
        status__in=[EnrollmentRequestStatus.PENDING, EnrollmentRequestStatus.PROCESSING]
    )
    approved_count = 0
    partial_count = 0
    errors = []
    ...
    for enrollment_request in approvable:
        try:
            ...
            if (
                ...
                and target
                and target.price
                and enrollment_request.price < target.price
            ):
                partial_count += 1
...
def reject_selected(modeladmin, request, queryset):
    ...
    for enrollment_request in rejectable:
        try:
            enrollment_request.reject(
                processed_by_user=request.user, reason="رفض جماعي من لوحة الإدارة"
            )
            rejected_count += 1
        except Exception as e:
            errors.append(f"{enrollment_request.get_participant()}: {str(e)}")
    ...
    if rejected_count:
        modeladmin.message_user(
            request, _(f"تم رفض {rejected_count} طلب"), level=messages.WARNING
        )
    if errors:
        modeladmin.message_user(
            request,
...
def mark_processing(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request, _(f"تم تحديد {updated} طلب كقيد المعالجة"), level=messages.INFO
    )
...
def extend_expiry(modeladmin, request, queryset):
    ...
    modeladmin.message_user(
        request, _(f"تم تمديد صلاحية {count} طلب لمدة 7 أيام"), level=messages.SUCCESS
    )
...
class EnrollmentRequestAdmin(ExcelExportMixin, admin.ModelAdmin):
            ...
            return (
                ...
                (
                    _("معلومات المشترك"),
                    {
                        ...
                        "description": _(
                            "اختر إما طالب أو طفل. عند اختيار طفل سيتم تعيين ولي الأمر تلقائياً."
                        ),
                    },
                ),
                (
                    _("الدفع (اختياري)"),
                    {
                        ...
                        "classes": ("collapse",),
                        "description": _(
                            "المبلغ يتم تعيينه تلقائياً من سعر الدورة إذا لم يُحدد"
                        ),
                    },
                ),
                (
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if obj is None:
            ...
            return (
                ...
                "student",
                "child",
                "parent",  # Lock participant info after creation
                "price"
            )
    ...
    def get_expiry_status(self, obj):
        ...
        if hours_left <= 24:
            return format_html(
                '<span style="color: #e67e22;" title="{}">🟡 {}س متبقية</span>',
                obj.expires_at.strftime("%Y/%m/%d %H:%M"),
                int(hours_left),
            )
        elif hours_left <= 72:
            ...
            return format_html(
                '<span style="color: #f39c12;" title="{}">🟡 {}ي متبقية</span>',
                obj.expires_at.strftime("%Y/%m/%d %H:%M"),
                days_left,
            )
        else:
            return format_html(
                '<span style="color: #27ae60;" title="{}">🟢 صالح</span>',
                obj.expires_at.strftime("%Y/%m/%d %H:%M"),
            )
    ...
    def get_processed_info(self, obj):
        ...
        if obj.processed_by:
            ...
            return format_html(
                '<small style="color: #7f8c8d;">👤 {}<br>{}</small>', name, date
            )
    ...
    def get_course_info(self, obj):
        ...
        if target:
            ...
            return format_html(
                ...
                target.capacity if hasattr(target, "capacity") else "",
            )
        return "-"
