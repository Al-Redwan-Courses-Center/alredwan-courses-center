...
class EnrollmentRequestCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (course is None and online_course is None) or (
            course is not None and online_course is not None
        ):
            raise serializers.ValidationError(
                "يجب تحديد إما الدورة الحضورية أو الدورة الإلكترونية فقط."
            )
        ...
        if user.role == "parent":
            ...
            if course and (not target_course.is_participant_eligible(child)):
                raise serializers.ValidationError(
...
class EnrollmentRequestApproveSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if (
            instance.course
            and instance.course.enrolled_count >= instance.course.capacity
        ):
            raise serializers.ValidationError(
