...
class OnlineCourseViewSet(viewsets.ReadOnlyModelViewSet):
        ...
        if request and request.user.is_authenticated:
            ...
            if student or child:
                ...
                qs = qs.prefetch_related(
                    Prefetch('video_lectures__watchprogress_set', queryset=watch_progress_qs, to_attr='prefetched_watch_progress')
                )
    ...
    def batch(self, request):
        ...
        if not ids:
            return Response([])
        id_list = [uuid.strip() for uuid in ids.split(',') if uuid.strip()]
