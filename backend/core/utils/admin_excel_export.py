...
class ExcelExportMixin:
    ...
    def direct_export_to_excel(self, request):
        ...
        if not filtered_queryset.exists():
            ...
            messages.warning(request, _('لا توجد بيانات للتصدير (No data to export)'))
    ...
    def export_to_excel(self, request, queryset):
        ...
        messages.success(
            request, 
            _('تم تصدير {} صف بنجاح (Successfully exported {} rows)').format(row_count, row_count)
        )
    ...
    def get_changelist_instance(self, request):
        ...
        if hasattr(self, 'get_search_help_text'):
            changelist_kwargs['search_help_text'] = self.get_search_help_text(request)
        else:
    ...
    def get_export_fields(self):
        ...
        if self.excel_export_fields:
            ...
            field_names = [f for f in self.list_display if f not in self.excel_export_exclude]
