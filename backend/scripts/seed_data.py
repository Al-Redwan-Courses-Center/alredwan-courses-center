...
from users.models.student_instructor_rating import (
    StudentInstructorRating, ParentInstructorRating,
    StudentCourseRating, ParentCourseRating
)
...
from courses.models import Season, Tag, Course, CourseSchedule, Lecture, Weekday, LectureStatus
...
from enrollments_payments.models import Enrollment, Payment, PaymentMethod, PaymentStatus
from enrollments_payments.models.enrollment_request import EnrollmentRequest, EnrollmentRequestStatus
from attendance.models import (
    LectureAttendance, InstructorAttendance, SupervisorSchedule,
    AttendanceStatus, AttendanceType, CheckInMethod, AttendanceDevice, FingerprintScanLog, ScanAction
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            instructors = self.create_instructors(options['instructors'], base_date, tags)
            ...
            parents, children = self.create_parents_and_children(options['parents'], base_date)

            print('Creating parent link requests...')
            self.create_parent_link_requests(parents, children)

            print('Creating courses...')
            courses = self.create_courses(options['courses'], base_date, seasons, instructors, tags)
            ...
            enrollments = self.create_enrollments(courses, students, children, base_date)
        ...
        self.print_summary(tags, seasons, instructors, students, parents, children, courses, enrollments)
    ...
    def clear_data(self):
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute('TRUNCATE TABLE attendance_fingerprintscanlog RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE attendance_instructorattendance RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE attendance_lectureattendance RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE attendance_supervisorschedule RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE attendance_attendancedevice RESTART IDENTITY CASCADE;')
            
            print('  Truncating exam tables...')
            cursor.execute('TRUNCATE TABLE courses_examresult RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE courses_exam RESTART IDENTITY CASCADE;')
            
            print('  Truncating enrollment tables...')
            cursor.execute('TRUNCATE TABLE enrollments_payments_enrollmentrequest RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE enrollments_payments_payment RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE enrollments_payments_enrollment RESTART IDENTITY CASCADE;')
            
            print('  Truncating rating tables...')
            cursor.execute('TRUNCATE TABLE users_studentinstructorrating RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE users_parentinstructorrating RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE users_studentcourserating RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE users_parentcourserating RESTART IDENTITY CASCADE;')
            
            print('  Truncating course tables...')
            cursor.execute('TRUNCATE TABLE courses_landingpagecourse RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE courses_lecture RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE courses_courseschedule RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE courses_course_tags RESTART IDENTITY CASCADE;')
            ...
            cursor.execute('TRUNCATE TABLE parents_parentlinkrequest RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE parents_childparents RESTART IDENTITY CASCADE;')
            ...
            cursor.execute('TRUNCATE TABLE users_landingpageinstructor RESTART IDENTITY CASCADE;')
            cursor.execute('TRUNCATE TABLE users_instructor_tags RESTART IDENTITY CASCADE;')
        ...
        if student_count > 0 or instructor_count > 0 or parent_count > 0:
            print(f'  ⚠️  Warning: Found orphaned records - Students: {student_count}, Instructors: {instructor_count}, Parents: {parent_count}')
    ...
    def create_instructors(self, count, base_date, tags):
        ...
        first_names = ['أحمد', 'محمد', 'عبدالله', 'خالد', 'عمر', 'فاطمة', 'عائشة', 'خديجة', 'مريم', 'زينب']
        last_names = ['الأحمدي', 'المحمدي', 'العبدلي', 'الخالدي', 'العمري', 'الفاطمي', 'السعيد', 'الرشيد', 'الكريم', 'الحسن']
        
        for i in range(count):
            # Create user
            phone = f'+20100{1000000 + i:07d}'
            dob = base_date - timedelta(days=random.randint(9000, 18000))  # 25-50 years old
    ...
    def create_attendance_devices(self):
        ...
        device_configs = [
            {'device_id': 'FP-MAIN-001', 'name': 'جهاز البصمة - المدخل الرئيسي', 'location': 'المدخل الرئيسي'},
            {'device_id': 'FP-CLASS-001', 'name': 'جهاز البصمة - قاعة 1', 'location': 'الطابق الأول - قاعة 1'},
            {'device_id': 'FP-CLASS-002', 'name': 'جهاز البصمة - قاعة 2', 'location': 'الطابق الأول - قاعة 2'},
            {'device_id': 'RFID-MAIN-001', 'name': 'جهاز RFID - المدخل', 'location': 'البوابة الرئيسية'},
        ]
    ...
    def create_students(self, count, base_date):
        ...
        first_names = ['علي', 'حسن', 'حسين', 'يوسف', 'إبراهيم', 'عمر', 'فاطمة', 'سارة', 'ليلى', 'نور']
        last_names = ['العلي', 'الحسن', 'اليوسف', 'الإبراهيمي', 'العمري', 'المحمدي', 'السعيد', 'الكريم', 'الرحيم', 'الجميل']
    ...
    def create_parents_and_children(self, count, base_date):
        ...
        parent_first_names = ['أبو أحمد', 'أبو محمد', 'أبو عبدالله', 'أم فاطمة', 'أم عائشة', 'أم خديجة']
        parent_last_names = ['الأحمدي', 'المحمدي', 'العبدلي', 'الخالدي', 'العمري']
        child_first_names = ['أحمد', 'محمد', 'فاطمة', 'عائشة', 'يوسف', 'مريم', 'زينب', 'علي']
        
        for i in range(count):
            phone = f'+20102{3000000 + i:07d}'
            dob = base_date - timedelta(days=random.randint(10000, 18000))  # 27-49 years old
            gender = random.choice(['male', 'female'])
            ...
            try:
                with transaction.atomic():
                    ...
                    if status == 'approved':
                        ...
                        ChildParents.objects.get_or_create(child=child, parent=requester)
                request_count += 1
            except Exception:
        ...
        if instructors:
            ...
            for idx, instructor in enumerate(featured_instructors):
                LandingPageInstructor.objects.create(
                    instructor=instructor,
                    order=len(featured_instructors) - idx
                )
                feature_count += 1
    ...
    def generate_lectures(self, courses):
        ...
        for course in courses:
            try:
                with transaction.atomic():  # savepoint – failure only rolls back this course
                    course.generate_lectures()

                    # Mark some past lectures as completed
                    past_lectures = course.lectures.filter(day__lt=timezone.now().date())
            ...
            for course in selected_courses:
                ...
                if days_range > 0:
                    ...
                    enrolled_at = timezone.make_aware(
                        datetime.combine(base_date - timedelta(days=enrolled_days_ago), time(10, 0))
                    )
                else:
    ...
    def create_enrollment_requests(self, courses, students, children, base_date):
        ...
        for student in random.sample(students, min(5, len(students))):
            ...
            available_courses = [c for c in active_courses if c.id not in enrolled_courses]
            ...
            if available_courses:
                ...
                EnrollmentRequest.objects.create(
                    course=course,
                    student=student,
                    price=course.price if random.random() < 0.7 else course.price * Decimal('0.5'),
                    status=status,
        ...
        for child in random.sample(children, min(5, len(children))):
            enrolled_courses = Enrollment.objects.filter(child=child).values_list('course', flat=True)
            available_courses = [c for c in active_courses if c.id not in enrolled_courses]
    ...
    def create_payments(self, enrollments, base_date):
        ...
        for enrollment in enrollments:
            ...
            if payment_scenario == 'full':
                ...
                paid_amount = Decimal(float(course_price) * paid_percentage).quantize(Decimal('0.01'))
                payment = Payment.objects.create(
                    enrollment=enrollment,
                    payer_student=enrollment.student if enrollment.student else None,
                    payer_parent=payer_parent,
                ...
                installment_amount = (course_price / num_installments).quantize(Decimal('0.01'))
    ...
    def create_exam_results(self, enrollments, base_date):
        ...
        for exam in past_exams:
            ...
            course_enrollments = Enrollment.objects.filter(course=exam.course, status='active')
            ...
            for enrollment in course_enrollments:
                ...
                if random.random() < 0.8:
                    ...
                    marks = (exam.total_marks * Decimal(percentage / 100)).quantize(Decimal('0.01'))
                    ...
                    ExamResult.objects.create(
                        ...
                        entered_at=exam.scheduled_at + timedelta(days=random.randint(1, 7))
                    )
                    result_count += 1
        ...
        for enrollment in enrollments:
            ...
            for lecture in past_lectures:
                ...
                attendance = LectureAttendance.objects.create(
                    ...
                    present=present,
                    rating=rating,
                    notes=random.choice([None, 'ممتاز', 'جيد جداً', 'يحتاج تحسين', None, None]),
                    marked_by=lecture.instructor.user if lecture.instructor else None,
                    marked_via='manual',
                    marked_at=timezone.make_aware(
                        datetime.combine(lecture.day, time(lecture.end_time.hour, lecture.end_time.minute))
                    ) if lecture.end_time else None,
                )
                attendance_count += 1
        ...
        for lecture in past_lectures:
            ...
            if lecture.status == LectureStatus.COMPLETED:
                ...
                check_in_time = timezone.make_aware(
                    datetime.combine(
                        lecture.day,
                        time(lecture.start_time.hour - random.randint(0, 1), 
                             random.randint(0, 59))
                    )
                ) if lecture.start_time else None
                ...
                ) if lecture.end_time and check_in_time else None
            ...
            while current_date <= min(base_date, active_season.end_date or base_date):
                if current_date.weekday() == schedule.day_of_week:
                    if random.random() < 0.95:
                        ...
                        if check_in_time > scheduled_start + timedelta(minutes=schedule.grace_period_minutes):
                            status = AttendanceStatus.LATE
                        else:
                            status = AttendanceStatus.PRESENT
                        
                        rating = Decimal(random.uniform(8.0, 10.0)).quantize(Decimal('0.01'))
                        device = random.choice(devices) if devices else None
    ...
    def print_summary(self, tags, seasons, instructors, students, parents, children, courses, enrollments):
        ...
        self.stdout.write(f'Supervisor Schedules:      {SupervisorSchedule.objects.count()}')
        self.stdout.write(f'Attendance Devices:        {AttendanceDevice.objects.count()}')
        ...
        self.stdout.write(f'Parent Link Requests:      {ParentLinkRequest.objects.count()}')
        self.stdout.write(f'Courses:                   {len(courses)}')
        self.stdout.write(f'Landing Page Features:     {LandingPageCourse.objects.count() + LandingPageInstructor.objects.count()}')
        self.stdout.write(f'Lectures:                  {Lecture.objects.count()}')
        self.stdout.write(f'Enrollments:               {len(enrollments)}')
        self.stdout.write(f'Enrollment Requests:       {EnrollmentRequest.objects.count()}')
        self.stdout.write(f'Payments:                  {Payment.objects.count()}')
        self.stdout.write(f'Exams:                     {Exam.objects.count()}')
        self.stdout.write(f'Exam Results:              {ExamResult.objects.count()}')
        self.stdout.write(f'Lecture Attendance:        {LectureAttendance.objects.count()}')
        self.stdout.write(f'Instructor Attendance:     {InstructorAttendance.objects.count()}')
        self.stdout.write(f'Fingerprint Scans:         {FingerprintScanLog.objects.count()}')
        self.stdout.write(f'Ratings (All):             {StudentInstructorRating.objects.count() + ParentInstructorRating.objects.count() + StudentCourseRating.objects.count() + ParentCourseRating.objects.count()}')
        self.stdout.write('='*60 + '\n')
        ...
        self.stdout.write('Supervisor:  +201001000000 / password123 (first 2 instructors)')
        self.stdout.write('='*60 + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base-date', type=str, default=None, help='Base date for seeding (YYYY-MM-DD)')
    parser.add_argument('--students', type=int, default=20, help='Number of students to create')
    parser.add_argument('--instructors', type=int, default=5, help='Number of instructors to create')
    ...
    command = Command()
    command.stdout = command.stderr = type('obj', (object,), {
