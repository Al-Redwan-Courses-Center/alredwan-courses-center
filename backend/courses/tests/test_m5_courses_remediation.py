...
class Milestone5CoursesRemediationTests(TestCase):
    ...
    def test_batch_schedules_retrieval(self):
        ...
        resp_single = self.client.get(f'/api/courses/schedules/?course_ids={self.course_1.id}')
        ...
        resp_multi = self.client.get(f'/api/courses/schedules/?course_ids={self.course_1.id},{self.course_2.id}')
    ...
    def test_secondary_parent_lecture_and_attendance_access(self):
        ...
        response = self.client.get(f'/api/courses/{self.course_1.id}/parent/{self.child.id}/lectures/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        unrelated_resp = self.client.get(f'/api/courses/{self.course_1.id}/parent/{self.child.id}/lectures/')
